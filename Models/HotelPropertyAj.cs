...
public class HotelPropertyAj
{
    ...
    public static string GetPropertyDesc(string rqbody, string ContextResult, string searchid, string HotelCode, string CurrencyCode, string b2c_idn)
    {
        string rq = "";
        string result = "";
        ...
        //result = XMLRead.ContextChange(searchid +"_propertydesc");

        //////Getting the Property Descriptions
        //if (result.ToString() != "")
        //{
        ...
        //        XMLRead.SaveXMLFile(rq, result, searchid + "_propertydesc_" + HotelCode + "_" + CurrencyCode);
        //    }
        // }
        ...
        //        DataTable dtMessageHeader = dsSession.Tables["MessageHeader"];
        //        string timestamp = DateTime.UtcNow.ToString();
        //        string cresult = XMLRead.closession(dtMessageData.Rows[0]["MessageId"].ToString(), timestamp, dtBinarySecurityToken.Rows[0]["BinarySecurityToken_Text"].ToString(), searchid + "_PropertyDesc" + HotelCode + "_" + CurrencyCode, XMLRead.pcc, XMLRead.ipcc);
        //    }
        //}
        return result;
    }
    ...
    TextInfo txtinfo;
    public HotelPropertyAj()
    {

    }
    public HotelPropertyAj(HPDCondition Hapc, string ContextResult, string searchid, string b2c_idn)
    {
        try
        {
            string Rqbody = GetRQ(Hapc);
            string result = GetPropertyDesc(Rqbody, ContextResult, searchid, Hapc.HotelCode, Hapc.CurrencyCode, b2c_idn);
    ...
    public HotelPropertyAj(string XmlResult, string HotelCode, string Searchid, string CurrencyCode, string b2c_idn)
    {
        XmlDataDocument xmldoc = new XmlDataDocument();
    ...
    private void PrepareRooms()
    {
        ...
        dtRoomInfo.Columns.Add("RatePernight", typeof(decimal));
        dtRoomInfo.Columns.Add("Taxes");
        dtRoomInfo.Columns.Add("CurrencyCode");
        dtRoomInfo.Columns.Add("TSurcharges_Amount");
        foreach (DataRow dr in dtRoomRate.Rows)
        {
            ...
            foreach (DataRow drt in drrates)
            {
                try
                {
                    ...
                    dri["RPH"] = dr["RPH"];
                    dri["CurrencyCode"] = drt["CurrencyCode"];
                    //need to get symbols for currency.
                    dri["Rate"] = drt["Amount"].ToString();
                    DataRow[] drHP = dtHotelPricing.Select("RRate_ID='" + drt["RRate_ID"].ToString() + "'");
                    ////  dri["HPTotalAmount"] = GetCurrencySymbol(drt["CurrencyCode"].ToString()) + drHP[i]["HPTotalAmount"].ToString();
                    dri["HPTotalAmount"] = drHP[0]["HPTotalAmount"].ToString();
                    dri["RatePernight"] = GetRatePerNight(drHP[0]["HPTotalAmount"].ToString());
                    dri["Taxes"] = GetTaxes(drHP[0]);
                    dri["TSurcharges_Amount"] = drHP[0]["TSurcharges_Amount"].ToString();
    ...
    private void CreateTables()
    {
        ...
        dtHotelPricing.Columns.Add("RRate_ID");
    ...
    private void AddXMLrate(XmlNode PNode, int ParentRno)
    {
        ...
        foreach (XmlNode xn in PNode.ChildNodes)
        {
            ...
            {
                AddXMLHotelTotalPricing(xn, rno);
            }
        }
    }
...
//rq += "<RatePlanCandidates><RateRange CurrencyCode='GBP'/></RatePlanCandidates>";